...
describe("DirectoryScanner", () => {
  ...
  beforeEach(async () => {
    ...
    fileSystemManager = new FileSystemManager(mockWorkspace.tempDir);
    
    // Stub the getPromptManagerPath method to return our test directory
    sinon.stub(fileSystemManager, "getPromptManagerPath").returns(mockWorkspace.testPromptPath);
    sinon.stub(fileSystemManager, "fileExists").callsFake((filePath: string) => {
      try {
  ...
  describe("scanDirectory() with options", () => {
    beforeEach(async () => {
      ...
      for (const filePath of testStructure) {
        ...
        await fs.promises.writeFile(fullPath, `# ${path.basename(filePath, '.md')}\nContent`);
      }
    });
    ...
    it("should respect excludePatterns option", async () => {
      ...
      const allFiles = [
        ...
        ...result.folders.flatMap(f => f.subfolders?.flatMap(sf => sf.prompts) || [])
      ];
      ...
      expect(allFiles.some(f => f.fileName.startsWith("."))).to.be.false;
      
      // Should include valid files
      expect(allFiles.some(f => f.fileName === "valid-prompt.md")).to.be.true;
      expect(allFiles.some(f => f.fileName === "nested-prompt.md")).to.be.true;
    });
    ...
    it("should respect maxDepth option", async () => {
      ...
      const checkMaxDepth = (folders: any[], currentDepth: number) => {
        for (const folder of folders) {
          ...
          if (folder.subfolders) {
            checkMaxDepth(folder.subfolders, currentDepth + 1);
          }
        }
      };
      ...
      checkMaxDepth(result.folders, 1);
       ...
       expect(allFiles.some(f => f.fileName === "deep-prompt.md")).to.be.false;
    });
    ...
    it("should apply both excludePatterns and maxDepth together", async () => {
      ...
      expect(allFiles.some(f => f.fileName.startsWith("."))).to.be.false;
      expect(result.folders.every(f => !f.subfolders || f.subfolders.length === 0)).to.be.true;
    });
  });
  ...
  describe("getAllPromptFiles()", () => {
    it("should return flat array of all prompt files", async () => {
      ...
      expect(allFiles.map(f => f.title)).to.include.members(["Root 1", "Root 2", "Sub 1"]);
    });
  });
  ...
  describe("findPromptFiles()", () => {
    ...
    it("should filter files using predicate", async () => {
      const codingFiles = await scanner.findPromptFiles(
        file => file.tags.includes("coding")
      );
    ...
    it("should return empty array when no files match", async () => {
      const noMatch = await scanner.findPromptFiles(
        file => file.tags.includes("nonexistent")
      );
  ...
  describe("error handling", () => {
    it("should handle filesystem errors gracefully", async () => {
      ...
      sinon.restore();
      sinon.stub(fileSystemManager, "getPromptManagerPath").returns("/invalid/path");
